...
Not sure what anything means? Pick **Help**: this whole guide is built into the app. **Up/Down** picks a topic, **A** opens it. Inside a page **Up/Down** scrolls, **LB + Up/Down** turns a page, **Left/Right** jumps between sections, **B** goes back to the topics.
...
- **Chop it:** on a phrase, **LB + Start** records it into a new sample ([render to sample](Samples#render-to-sample)). Play it backwards (`play reverse`, [play modes](Samples#play-modes)) or slice it for a fill.
