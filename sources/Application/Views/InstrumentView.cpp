...
void InstrumentView::GetGuideTopic(const char *&page, const char *&section) {
	...
	if (labPage_==INSTRUMENT_MOD_PAGE &&
	    (getInstrumentType()==IT_SAMPLE || isSynthLike(getInstrumentType()))) {
		...
		// Its own page; ENV, FILTER, MIX are explained there too, EQ on
		// the synth page
		if (labPage_==INSTRUMENT_EQ_PAGE) {
			page="synth";
			section="EQ - its own tone";
		} else {
			page="macro";
			switch(labPage_) {
