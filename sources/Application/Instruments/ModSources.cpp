...
static const int macroDests[]={
	MD_VOLUME,MD_CUTOFF,MD_RESO,MD_PITCH,MD_PAN,MD_FINE,MD_DRIVE,MD_TIMBRE,
	MD_COLOR,MD_REVERB,MD_DELAY,MD_CHORUS
} ;
#define SYNTH_DEST_COUNT ((int)(sizeof(synthDests)/sizeof(int)))
#define MACRO_DEST_COUNT ((int)(sizeof(macroDests)/sizeof(int)))
...
static char *macroDestNames[MACRO_DEST_COUNT] ;
...
void InstrumentMods::Create(VariableContainer &owner,int kind) {
	...
	} else if (kind==MIK_MACRO) {
		dests_=macroDests ;
		destCount_=MACRO_DEST_COUNT ;
		names=macroDestNames ;
	} else {
		dests_=sampleDests ;
		destCount_=SAMPLE_DEST_COUNT ;
