...
void Variable::SetString(const char *input, bool notify) {
    NAssert(input);
    switch (type_) {
    case FLOAT:
        value_.float_ = float(atof(input));
        break;
    case INT:
        value_.int_ =
            maxValue_.int_ ? MIN(atoi(input), maxValue_.int_) : atoi(input);
        break;
    case BOOL:
        value_.bool_ = (!strcmp("false", input) ? false : true);
        break;
    case STRING:
        stringValue_ = input;
        break;
